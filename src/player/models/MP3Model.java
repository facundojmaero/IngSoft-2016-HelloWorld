...
//		currentRepeatState.songFinished();
		Platform.runLater(() -> currentRepeatState.songFinished());
	}
	...
	public void seek(int progress){
		long bytes = new File(playlist.get(index)).length();
		try {
			player.seek(progress*bytes/getCurrentSongDurationSec());
		} catch (BasicPlayerException e) {
			e.printStackTrace();
		}
